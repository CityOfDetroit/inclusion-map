...
document.getElementById('geocoder').appendChild(geocoder.onAdd(map)); //================ geocoder for address search ends====================//
...
map.on('load', function () {
  ...
  var url = 'https://apis.detroitmi.gov/crowdsource/yahoo/wifi/locations/42.3314,-83.0458 ';
  fetch(url, {
    method: 'GET',
  ...
  .then(function (data) {
    ...
    for (var i = 0; i < data.length; i++) {
      var popup = new mapboxgl.Popup().setHTML('<div class="card mb-3">' + '  <div class="row no-gutters">' + '    <div class="col-md-4">' + '<img class="card-img" src="' + data[i].image_url + '"/>' + '    </div>' + '    <div class="col-md-8">' + '      <div class="card-body">' + '        <h6 class="card-title">' + data[i].name + '</h6>' + '<p class="place_address">' + '<a href="">' + data[i].location.address1 + ',' + data[i].location.city + ',' + data[i].location.state + ',' + data[i].location.zip_code + '</a>' + '</p>' + '<div class="rating" data-rating="' + data[i].rating + '"><div class="star"></div> <div class="star"></div> <div class="star"></div> <div class="star"></div> <div class="star"></div> </div>' + '      </div>' + '    </div>' + '  </div>' + '</div>');
      console.log(data[i].rating); // .setText(data[i].name);
